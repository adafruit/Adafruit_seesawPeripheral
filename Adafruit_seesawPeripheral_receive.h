...
void Adafruit_seesawPeripheral_processCommand(const uint8_t *packet,
  ...
  if (size < 2)
    return;
  uint8_t base = packet[0], reg = packet[1];
  ...
  if (base == SEESAW_STATUS_BASE) {
    if (reg == SEESAW_STATUS_SWRST && length == 1 && data[0] == 0xFF)
      Adafruit_seesawPeripheral_reset();
  } else if (base == SEESAW_GPIO_BASE) {
    if (reg == SEESAW_GPIO_BULK && length == 0) {
#if !defined(ARDUINO_ARCH_STM32)
    ...
    if (length != 4)
      return;
    uint32_t mask = Adafruit_seesawPeripheral_read32(data) & VALID_GPIO;
    switch (reg) {
    case SEESAW_GPIO_DIRSET_BULK:
      Adafruit_seesawPeripheral_gpioDirection(mask, true);
      break;
    case SEESAW_GPIO_DIRCLR_BULK:
      Adafruit_seesawPeripheral_gpioDirection(mask, false);
      break;
    case SEESAW_GPIO_BULK:
      Adafruit_seesawPeripheral_gpioWrite(VALID_GPIO & ~mask, false);
      Adafruit_seesawPeripheral_gpioWrite(mask, true);
      break;
    case SEESAW_GPIO_BULK_SET:
      Adafruit_seesawPeripheral_gpioWrite(mask, true);
      break;
    case SEESAW_GPIO_BULK_CLR:
      Adafruit_seesawPeripheral_gpioWrite(mask, false);
      break;
    case SEESAW_GPIO_BULK_TOGGLE:
      Adafruit_seesawPeripheral_gpioToggle(mask);
      break;
    case SEESAW_GPIO_PULLENSET:
      Adafruit_seesawPeripheral_gpioPull(mask, true);
      break;
    case SEESAW_GPIO_PULLENCLR:
      Adafruit_seesawPeripheral_gpioPull(mask, false);
      break;
    case SEESAW_GPIO_INTENSET:
      Adafruit_seesawPeripheral_gpioInterrupt(mask, true);
      break;
    case SEESAW_GPIO_INTENCLR:
      Adafruit_seesawPeripheral_gpioInterrupt(mask, false);
      break;
    }
#if CONFIG_ADC
...
#else
      g_bufferedADCRead = analogRead(pin);
#endif
      g_adcStatus = 0;
    }
#endif
    ...
    if (reg == SEESAW_NEOPIXEL_PIN && length == 1) {
      ...
      uint16_t count = Adafruit_seesawPeripheral_read16(data);
      g_neopixel_bufsize = min(count, (uint16_t)CONFIG_NEOPIXEL_BUF_MAX);
    ...
    case SEESAW_ENCODER_INTENCLR: {
      bool enabled = (reg & 0xF0) == SEESAW_ENCODER_INTENSET;
#if defined(ARDUINO_ARCH_STM32)
      if (enabled)
...
#endif
        g_enc_value[index] = (int32_t)Adafruit_seesawPeripheral_read32(data);
#if defined(ARDUINO_ARCH_STM32)
        g_enc_delta[index] = 0;
        interrupts();
#endif
