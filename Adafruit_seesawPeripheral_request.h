...
void requestEvent(void) {
  ...
  uint8_t base = selectedBase, reg = selectedRegister;
#else
  uint8_t base = i2c_buffer[0], reg = i2c_buffer[1];
#endif
  ...
  if (base == SEESAW_STATUS_BASE) {
    if (reg == SEESAW_STATUS_HW_ID)
      Wire.write((uint8_t)SEESAW_HW_ID);
    else if (reg == SEESAW_STATUS_VERSION)
      Adafruit_seesawPeripheral_write32(CONFIG_VERSION);
    else if (reg == SEESAW_STATUS_OPTIONS) {
      uint32_t options =
...
#if CONFIG_INTERRUPT || defined(ARDUINO_ARCH_STM32)
    else if (reg == SEESAW_GPIO_INTFLAG) {
...
#endif
#if CONFIG_ADC
  } else if (base == SEESAW_ADC_BASE) {
    if (reg >= SEESAW_ADC_CHANNEL_OFFSET)
      Adafruit_seesawPeripheral_write16(g_bufferedADCRead);
    else if (reg == SEESAW_ADC_STATUS)
      Wire.write(g_adcStatus);
#endif
#if CONFIG_PWM || CONFIG_PWM_16BIT
  } else if (base == SEESAW_TIMER_BASE) {
    Wire.write(g_pwmStatus);
#endif
#if CONFIG_EEPROM
  } else if (base == SEESAW_EEPROM_BASE) {
#if defined(ARDUINO_ARCH_STM32)
    uint8_t value = 0xFF;
    if (eepromSafe)
      value = *((const uint8_t *)eepromAddress + reg);
    Wire.write(value);
#else
    Wire.write(EEPROM.read(reg % EEPROM.length()));
#endif
#endif
#if CONFIG_NEOPIXEL
    ...
    if (reg == SEESAW_SPECTRUM_RESULTS_LOWER) {
      Wire.write(fht_log_out, 32);
    } else if (reg == SEESAW_SPECTRUM_RESULTS_UPPER) {
      Wire.write(&fht_log_out[32], 32);
    } else if (reg == SEESAW_SPECTRUM_CHANNEL) {
    ...
    } else if (reg == SEESAW_SPECTRUM_RATE) {
