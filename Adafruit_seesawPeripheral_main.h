...
void Adafruit_seesawPeripheral_pinChangeDetect(void) {
  ...
  for (uint8_t encodernum = 0; encodernum < CONFIG_NUM_ENCODERS; encodernum++) {
    ...
    if (encodernum == 0) {
      enc_cur_pos |= ((BIT_IS_CLEAR(in, CONFIG_ENCODER0_A_PIN)) << 0) |
                     ((BIT_IS_CLEAR(in, CONFIG_ENCODER0_B_PIN)) << 1);
    ...
    if (encodernum == 1) {
      enc_cur_pos |= ((BIT_IS_CLEAR(in, CONFIG_ENCODER1_A_PIN)) << 0) |
                     ((BIT_IS_CLEAR(in, CONFIG_ENCODER1_B_PIN)) << 1);
    }
#endif
#if defined(CONFIG_ENCODER2_A_PIN)
    if (encodernum == 2) {
      enc_cur_pos |= ((BIT_IS_CLEAR(in, CONFIG_ENCODER2_A_PIN)) << 0) |
                     ((BIT_IS_CLEAR(in, CONFIG_ENCODER2_B_PIN)) << 1);
    }
#endif
#if defined(CONFIG_ENCODER3_A_PIN)
    if (encodernum == 3) {
      enc_cur_pos |= ((BIT_IS_CLEAR(in, CONFIG_ENCODER3_A_PIN)) << 0) |
                     ((BIT_IS_CLEAR(in, CONFIG_ENCODER3_B_PIN)) << 1);
    }
#endif
